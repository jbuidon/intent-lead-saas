...
function setSearchUI(state) {
  ...
  if (state === 'searching') {
    ...
    // Show empty results container with "waiting for first lead" message
    if (searchLeads.length === 0) {
      document.getElementById('searchResults').innerHTML =
        `<div class="loading-state">
          <div class="pulse-ring"></div>
          <p>Scanning for buyers… leads will appear here as they're found.</p>
          <p class="loading-sub">Click ⏹ Stop anytime to cancel</p>
        </div>`;
    }
  } else {
...
function updateProgress(current, total) {
  ...
  if (label) label.textContent = `${current} / ${total} queries`;
}
...
function appendLead(lead, containerId, providerLabel) {
  ...
  // Remove the loading/empty state on first lead
  ...
  if (countEl) {
    ...
    countEl.textContent = (current + 1) + ' Lead' + (current + 1 !== 1 ? 's' : '');
  }
}
